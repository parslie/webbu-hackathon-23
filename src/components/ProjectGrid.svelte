    ...
    <div class="grid absolute" style="opacity: {$gridOpacity};">
        {#each projects as project}
            {#if project !== selectedProject}
                <article class="preview" on:click={(e) => onProjectSelect(e, project)} on:keydown={() => {}}>
                    <h1>{project.title}</h1>
                    <p>{project.description}</p>
                </article>
            {:else}
                <article class="preview" on:click={(e) => onProjectSelect(e, project)} on:keydown={() => {}} style="visibility: hidden;">
                    <h1>{project.title}</h1>
                    <p>{project.description}</p>
                </article>
            {/if}
